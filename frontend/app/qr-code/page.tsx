...
export default function QRCodePage() {
  ...
  const attendeeData = {
    name: user?.user_metadata?.full_name || user?.user_metadata?.name || user?.email?.split('@')[0] || 'John Doe',
    email: user?.email || 'john.doe@example.com',
    company: user?.user_metadata?.company || 'Tech Innovations',
    attendeeId: user?.id?.slice(0, 12).toUpperCase() || 'ATT-2025-1234'
  };
